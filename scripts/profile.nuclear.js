...
(function () {
  ...
  Object.assign(dbg.style, {
    ...
    maxWidth: "420px", maxHeight: "40vh", overflow: "auto",
    background: "rgba(20,20,20,.95)", color: "#ddd", font: "12px/1.4 ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace",
    padding: "10px 12px", borderRadius: "10px", boxShadow: "0 4px 24px rgba(0,0,0,.4)"
  });
  dbg.innerHTML = "<div style='font-weight:700;color:#8bdc8b'>SE Debug</div>";
  document.addEventListener("DOMContentLoaded", () => document.body.appendChild(dbg));
  function log(...a) { dbg.insertAdjacentHTML("beforeend", `<div>• ${a.map(x => typeof x === "string" ? x : JSON.stringify(x)).join(" ")}</div>`); }
  function fail(msg) { dbg.insertAdjacentHTML("beforeend", `<div style="color:#ff9b9b">✗ ${msg}</div>`); }
  function ok(msg) { dbg.insertAdjacentHTML("beforeend", `<div style="color:#8bdc8b">✓ ${msg}</div>`); }

  // Global error surfaces into the panel
  window.addEventListener("error", (e) => fail(`JS Error: ${e.message}`));
  window.addEventListener("unhandledrejection", (e) => fail(`Promise rejection: ${e.reason && e.reason.message ? e.reason.message : e.reason}`));

  // ===== Quick sanity checks that commonly cause “nothing loads” =====
  document.addEventListener("DOMContentLoaded", () => {
    const reqIds = ["profileName", "publicationsList", "trendCharts", "careerTimeline", "tagsContainer", "pubSort", "totalWorks"];
    reqIds.forEach(id => {
      if (!document.getElementById(id)) fail(`Missing DOM node: #${id} (check profile.html IDs)`);
    });
    ok("DOM ready. profile.nuclear.js is running.");
  });
