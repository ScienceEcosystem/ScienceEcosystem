...
// Goes through /api/paper/living-evidence rather than fetching evidence.json
// straight off raw.githubusercontent.com, so this page works the same way
// whether the repo committed its own manifest (author-published, or
// ci-verified if a workflow re-runs it) or has none at all and
// ScienceEcosystem generated one itself by parsing the repo's source — see
// server/living-paper-cache.js. Returns { tier, evidence } — evidence is
// null when tier is "none".
async function fetchEvidence(repo) {
  const res = await fetch(`/api/paper/living-evidence?repo=${encodeURIComponent(repo)}`);
  if (!res.ok) return { tier: 'none', evidence: null };
  return res.json();
}

const TIER_LABELS = {
  'ci-verified': { text: 'CI-verified', title: 'This repository has a GitHub Actions workflow that re-generates evidence.json — the manifest is checked against a live re-run, not just a one-time local export.' },
  'author-published': { text: 'Author-published', title: 'The author committed this evidence.json to the repository. There is no visible CI workflow re-running it, so there is no guarantee it still matches the current code.' },
  'auto-generated': { text: 'Auto-generated by ScienceEcosystem', title: 'No evidence.json was found in this repository. ScienceEcosystem generated this manifest itself by parsing the repository’s source files — nobody has re-run this code to confirm it, so treat it as a best-effort index, not a verified claim.' },
};

function tierBadgeHtml(tier) {
  const info = TIER_LABELS[tier];
  if (!info) return '';
  const cls = tier === 'auto-generated' ? 'badge-neutral' : (tier === 'ci-verified' ? 'badge-ok' : 'badge');
  return `<span class="badge ${cls}" style="font-size:.7rem;vertical-align:middle;" title="${escapeHtml(info.title)}">${escapeHtml(info.text)}</span>`;
}
...
async function loadLinksAndResearchObjects(repo) {
  ...
  linksEl.innerHTML = `
    ...
    <div class="reference-item" onclick="window.open('/api/paper/living-evidence?repo=${encodeURIComponent(repo)}','_blank')">
      <strong style="font-size:.85rem;">evidence.json</strong>
      <p class="muted small">Raw claim → code/data manifest — via ScienceEcosystem, works whether the repo committed its own or not</p>
    </div>
  `;
...
async function init() {
  ...
  try {
    const [html, { tier, evidence }] = await Promise.all([
    ...
    frame.addEventListener('load', () => {
      ...
      const badge = tierBadgeHtml(tier);
      const pct = total ? Math.round((linked / total) * 100) : 0;
      const summary = evidence
        ? `<div class="lp-coverage"><strong>${linked}/${total}</strong>&nbsp;claims linked to code &amp; data (${pct}%)</div>${badge ? `<div style="margin-top:.4rem;">${badge}</div>` : ''}`
      ...
      coverageInline.innerHTML = evidence
        ? `<strong>${linked}/${total}</strong>&nbsp;claims linked ${badge}`
