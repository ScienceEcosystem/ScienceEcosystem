...
(function () {
  ...
  async function loadCitationContexts(doi) {
    ...
    try {
      ...
      // Only genuine Semantic Scholar extracts are informative about HOW a
      // paper is cited — the OpenAlex-abstract fallback (source ===
      // "openalex_abstract") is just that paper's abstract opening, unrelated
      // to the actual citing sentence, so it doesn't belong in this section
      ...
      if (introEl) introEl.innerHTML = `The exact sentences where other papers cite this work — <strong id="citationCount">${contexts.length}</strong> citation contexts found via Semantic Scholar:`;
