...
(function () {
  ...
  function renderJournalBlockSimple(p, source){
    ...
    else if (percent >= 70) { grade = "Good"; color = "#00729f"; }
    ...
    $("journalBlock").innerHTML =
      ...
      '<details style="margin-top:1rem;">' +
        '<summary style="cursor:pointer; color:#00729f; font-size:0.9rem;">How is this calculated?</summary>' +
  ...
  function renderReproducibilityScore(paperData, researchObjects){
    ...
    block.innerHTML = '' +
      ...
      '<div class="repro-score" style="text-align:center; margin-bottom:1rem;">' +
        ...
        '</div>' +
      '</div>' +
      (result.note ? '<p style="font-size:0.85rem; color:#00729f; background:#f0f8ff; padding:0.75rem; border-radius:4px; margin-bottom:1rem;">'+escapeHtml(result.note)+'</p>' : '') +
