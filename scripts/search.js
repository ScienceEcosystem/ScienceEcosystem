...
(function () {
  ...
  function addMailto(u){
    try { var url = new URL(u, API); if(!url.searchParams.get("mailto")) url.searchParams.set("mailto", MAILTO); return url.toString(); }
    catch(_) { return u; }
  }
  ...
  async function fetchPapers(query, authorIds, page){
    var collated = [];
    try {
      // author-scoped pulls (use tail IDs)
    ...
    for (var i=0;i<works.length;i++){
      if (useComp) {
        try{
          ...
          console.warn("renderPaperCard failed, fallback", e);
          useComp = false; i--; continue;
        }
      } else {
        var w = works[i] || {};
  ...
  async function runSearch(query){
    ...
    // authors first (for author-scoped boosts)
    var authors = [];
    ...
    try {
      ...
      if (!papers.length && (totalResults||0) === 0) {
        $("papersList").innerHTML = '<p class="muted">No papers found.</p>';
      } else {
  ...
  window.handleUnifiedSearch = function(){
    ...
    if (!q) return;
    try { runSearch(q); } catch (e) { console.error(e); }
  };
